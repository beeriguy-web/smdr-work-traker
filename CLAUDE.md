...
- The company logo (`.header-logo` in the on-screen `.form-header`, and its inline-styled twin in `buildPdfHTML`'s header) is a base64-encoded PNG data URI embedded directly in the markup — cropped/cleaned from a photo of smdrtech.co.il and not a vector asset, so avoid re-scaling it much larger than its current ~46px display size or JPEG-era artifacts become visible. As with all PDF-template styling, keep the two copies in sync if the logo or its layout changes.
